...
export const AnimatedTestimonials = ({
	...
	return (
		...
		>
			<div
				className={[
					"relative grid grid-cols-1 gap-20",
					showContent ? "md:grid-cols-2" : "",
				].join(" ")}
			>
				<div>
					<div
						className={["relative h-80 w-full", imageContainerClassName].join(
							" ",
						)}
					>
						<AnimatePresence>
					...
					{activeTestimonial && renderOverlay ? (
						<div className="pointer-events-none absolute inset-0">
							{renderOverlay(activeTestimonial, active)}
						</div>
					) : null}
				</div>
				...
				{showContent ? (
						...
						>
							...
							<motion.p className="mt-8 text-lg text-gray-500 dark:text-neutral-300">
								{testimonials[active].quote.split(" ").map((word, index) => (
									<motion.span
										// biome-ignore lint/suspicious/noArrayIndexKey: per-word reveal animation needs stable positional key
					...
					<div className={["flex gap-4 pt-4", controlsClassName].join(" ")}>
						<button
							type="button"
							onClick={handlePrev}
